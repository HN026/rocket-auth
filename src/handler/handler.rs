...
use crate::models::models::{SignupUser, LoginUser, User, NewUser};
...
pub async fn signup<'a>(conn: DbConn, signup_user: Json<SignupUser<'a>>) -> Json<Result<User, String>> {
...
pub async fn signin<'a>(conn: DbConn, login_info: Json<LoginUser<'a>>) -> Json<Option<User>> {
    ...
    let result = conn.run(move |c| {
        users.filter(username.eq(username_clone))
            .first::<User>(c)
            .optional()
    }).await;
